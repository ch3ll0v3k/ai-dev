...
const double LR_IH = 0.7;  // 0.7;
const double LR_HO = 0.03; // 0.07;

const double MIN_ERROR = 0.0001;
//const double BIAS = 1.05;
...
void calcNet(void) {
    ...
    for (i = 0; i<numHidden; i++) {
        ...
        for (int j = 0; j<numInputs; j++) {
            hiddenVal[i] += (trainInputs[patNum][j] * weightsIH[j][i]) + BIAS ;
        }
